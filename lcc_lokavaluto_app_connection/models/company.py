...
class Company(models.Model):
    ...
    def _cron_import_new_digital_currency_debit_requests(self):
        ...
        for transaction in transactions:
            ...
            if len(debit_requests) > 0:
                _logger.info(
                    "Debit request already exists for transaction %s"
            ...
            _logger.info(
                ...
                % (request.wallet_id, transaction.get("tx_id"))
            )
    ...
    def _build_debit_request_values(self, transaction):
        wallet_ids = self.env["res.partner.backend"].search(
            [("name", "=", transaction.get("sender"))]
        )
        if len(wallet_ids) == 0:
            _logger.error(
                "No wallet found for debit transaction %s, debit request creation CANCELLED"
                % transaction.get("tx_id")
            )
            return {}
        elif len(wallet_ids) > 1:
            _logger.error(
                "Too many wallets found for debit transaction %s, debit request creation CANCELLED"
                % transaction.get("tx_id")
            )
            return {}
        values = {
